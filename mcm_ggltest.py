...
odir = (
    f"{conf['odir']}/ggl-nside{conf['nside']}"
    f"_{os.path.basename(conf['elledges']).replace('.txt', '')}"
)
print(conf, odir)

bins = nmt.NmtBin.from_edges(
    *np.loadtxt(conf["elledges"], unpack=True, dtype="i4")
)
...
if conf["type"] == "flask":
    ...
    cshcat_fn = [
        f"{conf['flaskdir']}/srccat_z{iz+1}_s{iseed}_ck{ick}.parquet"
        for iz in range(conf["nz_source"])
    ]
    cshcat = [csh.cat_fromflsk(fn, conf["nside"]) for fn in cshcat_fn]
    #    ofn = f'{odir}/cls_ggl_s{iseed}_ck{ick}.npz'

    #    field_i = []
    print(f"getting dmask ...")
    dmask_i = hp.read_map(
        f'{conf["ck_dir"]}/ck{ick}_nside{conf["nside"]}.fits'
    )
    ...
    for j in range(conf["nz_source"]):
        w = mcm_process(
            ick, j + 1, dmask_i, cshmask_j[j], bins, conf["mcm_dir"]
        )
