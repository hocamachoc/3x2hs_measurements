...
def cshcat_make(
    map_fn, nbar, sigma_e, fgoodmap_fn, fgood_thold=0.0, seed=None
):
    """
    Generates a cosmic-shear catalog for a FLASK simulation realization.
    """
    cshcat = pd.DataFrame(
        columns=["ra", "dec", "g1", "g2", "g1_true", "g2_true"]
    )
...
def ggmap_sample_positions(
    ip_good, nside, nbar, fgoodmap, seed=None, nside_up=4
):
    """
    ...
    ra, dec = ncmap_sample_positions(
        nc_map, ip_good, nside, fgoodmap, nside_up=nside_up
    )
...
def process_one_kggmap(
    iseed, ick, iz, flaskdir, outdir, nbar, sigma_e, kggpref="kappa-gamma-f10"
):
    """
...
def lnscat_make_frompmap(map_fn, fgoodmap_fn, fgood_thold=0.0, seed=None):
    ...
    nc_map, ip_good, nside_map, fgoodmap = pmap_load(
        map_fn, fgoodmap_fn, fgood_thold=fgood_thold
    )
...
def load_inputcl_y1(
    i, j, lmax, idir="/global/cscratch1/sd/hcamacho/flask_desy1/Cl_flask"
):
    """
    Loads FLASK input Cls to a numpy array
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("conf", nargs=1, help="YAML configuration file")
    parser.add_argument(
        "--iseed", type=int, required=True, help="Seed index (0-based)."
    )
    parser.add_argument(
        "--des_release", required=True, help="y1 or y3 for the moment"
    )
    parser.add_argument(
        "--processes",
        type=int,
    ...
    if o.des_release == "y1":
        ...
        args = [
            ...
            for iz, ick in it.product(
                range(conf["nz_lns"]), range(conf["nck"])
            )
        ]
        ...
        raise NotImplementedError(
            "Currently only y1 and y3 des_release(s) " + "are supported."
        )
    ...
    args = [
        ...
        for iz, ick in it.product(range(conf["nz_src"]), range(conf["nck"]))
    ]
    with mp.Pool(processes=o.processes) as pool:
