...
def cls_flask(iseed, ick, ell_ini, ell_end, nside, nz, flaskdir, maskdir, out_dir):
    ...
    for zi in range(5):
        cls[f'cl_{zi}'], cls[f'nl_{zi}'] = _get_cl(
            zi, cat, msk, w, nside, ipgood)
    return cls
...
def _gc_field(nc, msk, ipgood):
    ...
    dmap[ipgood] = nc[ipgood] / msk[ipgood] \
        * msk[ipgood].sum() / nc[ipgood].sum() - 1
